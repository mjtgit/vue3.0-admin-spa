...
export default {
  ...
  methods: {
    ...
    submitForm() {
        ...
        that.editable = false;
        var par = {
            "account":  that.personalInfo.account,
            "nickname": that.personalInfo.nickname,
            "trueName": that.personalInfo.trueName,
            "sex": that.personalInfo.sex,
            "email": that.personalInfo.email,
            "introduction": that.personalInfo.introduction,
        };
    ...
    resetForm() {
        var that = this;
        that.editable = false;
        that.personalInfo = JSON.parse( JSON.stringify(that.personalInfo_init) ) ;
    },
    //上传头像
    uploadAvatar(){
        alert('稍后补上')
