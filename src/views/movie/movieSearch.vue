<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input @keyup.enter.native="handleFilter" style="width: 200px;margin-right:10px;" class="filter-item" placeholder="明星、导演姓名" v-model="listQuery.name">
      </el-input>

      <el-input @keyup.enter.native="handleFilter" style="width: 200px;margin-right:10px;" class="filter-item" placeholder="电影类型" v-model="listQuery.type">
      </el-input>

      <el-select clearable class="filter-item" style="width: 130px;margin-right:10px;" v-model="listQuery.type" placeholder="类型">
...
export default {
  ...
  methods: {
    ...
    getList() {
        ...
        //请求参数
        let par = {
            "count": that.listQuery.pageSize,
            "start": (that.listQuery.currPage-1)*that.listQuery.pageSize,
            "q": that.listQuery.name,         //明星、导演姓名
            "tag": that.listQuery.type,       //类型
        };
        // jsonp请求方式
        that.$http.jsonp('https://api.douban.com/v2/movie/search',{params: par}).then(res => {
            // console.log(res)
            // success callback
            var data = res.body;
            // console.log(data)
            if(data.subjects){
                that.list = data.subjects;
                // console.log('列表数据：',that.list);
                // that.listQuery.currPage = data.start + 1; //页数开始
                // that.listQuery.pageSize = data.count;     //每页多少条
    ...
    handleCreateSubmit(){
        var that = this;
        // console.log('新增入参：',that.temp,that.list)
        //这里作演示用，正式新增 请提交到接口
