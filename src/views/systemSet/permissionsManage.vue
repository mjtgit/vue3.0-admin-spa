    ...
    <el-dialog title="设置权限" :visible.sync="dialogPermissionsVisible">
      <el-form class="small-space" :model="roleTemp" label-position="left" label-width="70px" style='width: 100%; '>
       <!-- <el-checkbox-group v-model="smMenuBeanDtoList"> -->
          <el-checkbox v-for="(item,index) in smMenuBeanDtoList" label="item.url" name="type" style="margin:0 15px 15px 0;"  v-model="item.set" :key="index">{{ item.menuName }}</el-checkbox>
        <!-- </el-checkbox-group> -->
      </el-form>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    getList() {
        ...
        // })
        global.fetchGet(api.roleAndUser,that.listQuery).then(res =>{
    ...
    setPermissions(index,item) {
        ...
        // console.log(item);
        var that = this;
        global.fetchGet(api.getMenuAndElement,{roleId:item.smRoleBeanDto.id}).then(res =>{
            // console.log('-------获取到数据：',JSON.stringify(res) )
            // console.log('-------获取到数据：',res )
            var data = res;
            if(data.resultCode == 0){
                that.smMenuBeanDtoList = data.data.smMenuBeanDtoList;
                that.dialogPermissionsVisible = true;
                // console.log('列表数据：',that.smMenuBeanDtoList);
            }else{
                Message({
                    showClose: true,
                    message: res.resultMsg,
    ...
    setPermissionsSubmit(){
        var that = this;
        // console.log( JSON.stringify(that.smMenuBeanDtoList) );
        //周转成前端所需格式 提交到vuex中。实际开发去掉以下代码，把以上参数提交给接口即可。2017-7-9
        var permissions = {};
        for (var i = 0 , len = that.smMenuBeanDtoList.length; i < len; i++) {
            permissions[that.smMenuBeanDtoList[i].url] = that.smMenuBeanDtoList[i].set;
        }
        that.$message({
            showClose: true,
    ...
    handleCreateSubmit(){
        var that = this;
        // console.log('新增入参：',that.roleTemp)
        var item = {
            "smRoleBeanDto":{"roleName":that.roleTemp.roleName}
        }
        //这里作演示用，正式新增 请提交到接口
        that.list.push(item)
